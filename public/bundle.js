var app = (function () {
    ...
    /* src/components/Board.svelte generated by Svelte v3.4.4 */
    ...
    function create_each_block(ctx) {
    	var div, div_number_value, t, if_block_anchor, dispose;
    	...
    	return {
    		c: function create() {
    			...
    			div.className = "trout svelte-q77wyd";
    			attr(div, "number", div_number_value = ctx.f);
    		...
    		p: function update(changed, new_ctx) {
    			ctx = new_ctx;
    			if ((changed.field) && div_number_value !== (div_number_value = ctx.f)) {
    				attr(div, "number", div_number_value);
    			}
    ...
    const troutClick = (i) => {
        if (field[i] === 0) {
            ...
            const queen = smicleUtil_1(64).filter(n => field[n] == 23)._delete(i);
            resetBoard();
    ...
    const colorTheSquares = (i) => {
        changeToKaoriColor(i);
        paintTheTrout(i)
            .filter(n => field[n] === 0)
            .forEach(changeToKaedeColor);
    };
    const resetBoard = () => smicleUtil_1(64).forEach(i => { const $$result = field[i] = 0; $$invalidate('field', field); return $$result; });
    const changeToKaedeColor = (i) => { const $$result = field[i] = field[i] === 0 ? 25 : field[i]; $$invalidate('field', field); return $$result; };
    const changeToKaoriColor = (i) => { const $$result = field[i] = field[i] === 0 ? 23 : field[i]; $$invalidate('field', field); return $$result; };
    ...
    class Board extends SvelteComponentDev {
    	constructor(options) {
    		...
    		if (ctx.count === undefined && !('count' in props)) {
    			console.warn("<Board> was created without expected prop 'count'");
    		}
    		if (ctx.judge === undefined && !('judge' in props)) {
    			console.warn("<Board> was created without expected prop 'judge'");
    		}
    		if (ctx.field === undefined && !('field' in props)) {
    			console.warn("<Board> was created without expected prop 'field'");
    		}
    	}
    ...
    function create_fragment$1(ctx) {
    	...
    	board.$on("updateChild", ctx.updateChild);
    	...
    	return {
    		c: function create() {
    			...
    			add_location(span0, file$1, 4, 4, 143);
    			button.className = "svelte-1bowyze";
    			add_location(button, file$1, 4, 32, 171);
    			add_location(br, file$1, 4, 82, 221);
    			span1.className = "svelte-1bowyze";
    			add_location(span1, file$1, 5, 4, 230);
    			main.className = "svelte-1bowyze";
    			add_location(main, file$1, 1, 2, 25);
    			div.className = "centered svelte-1bowyze";
    			add_location(div, file$1, 0, 0, 0);
    			dispose = listen(button, "click", ctx.fromTheBeginning);
    		},
    		...
    		p: function update(changed, ctx) {
    			var board_changes = {};
    			if (changed.count) board_changes.count = ctx.count;
    			if (changed.judge) board_changes.judge = ctx.judge;
    			if (changed.field) board_changes.field = ctx.field;
    		...
    		i: function intro(local) {
    			if (current) return;
    			board.$$.fragment.i(local);

    			current = true;
    		},

    		o: function outro(local) {
    			board.$$.fragment.o(local);
    			current = false;
    		},
